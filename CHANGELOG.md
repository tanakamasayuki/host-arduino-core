...
## Unreleased
- (EN) Added `tests/interop/http_redirect` and `tests/interop/http_chunked`. The redirect test fires `https://httpbin.org/redirect/3` with `HTTPC_FORCE_FOLLOW_REDIRECTS` and verifies the `X-Test-Tag` request header survives the three 302 hops and surfaces in the final 200 body. The chunked test hits `https://httpbin.org/stream/3`, asserts `getSize() == -1` (the chunked-encoding code path) and counts the tag appearing exactly 3 times in the decoded body. Together they validate that the redirect-following loop and chunked-decode produce identical results on the host (OpenSSL) and real ESP32 (mbedTLS) backends.
- (JA) `tests/interop/http_redirect` と `tests/interop/http_chunked` を追加。redirect テストは `https://httpbin.org/redirect/3` を `HTTPC_FORCE_FOLLOW_REDIRECTS` で叩き、3 ホップの 302 を跨いで `X-Test-Tag` リクエストヘッダが最終 200 body に残ることを確認。chunked テストは `https://httpbin.org/stream/3` を叩き、`getSize() == -1`（chunked エンコード経路を通った証拠）と decode 後 body 内にタグがちょうど 3 回出現することを確認。host (OpenSSL) と ESP32 (mbedTLS) でリダイレクトループと chunked decode が同等に動くことを検証。
