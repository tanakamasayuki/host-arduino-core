// WiFiClientSecure end-to-end probe.
//
// Reads "CONNECT <port>" on Serial, connects via TLS to
// 127.0.0.1:<port>, sends "PING", echoes whatever the server sends
// back over Serial as "RX <byte>" lines.
//
// Requires the `TLS=OpenSSL` board menu option.
...
void setup()
{
    ...
    client.setInsecure(); // explicit, even though host always skips verification
