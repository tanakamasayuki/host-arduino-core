...
def test_http_redirect(dut):
    ...
    try:
        dut.expect(re.compile(rb"READY"), timeout=180)

        # 1. DISABLE: 302 surfaces as the status code; Location header is exposed.
        dut.write(b"MODE 0\n")
        dut.expect(re.compile(rb"MODE_OK 0"), timeout=180)
        dut.write(f"GET {base}/once\n".encode())
        dut.expect(re.compile(rb"CODE=302"), timeout=180)
        dut.expect(re.compile(rb"LOC=/target"), timeout=180)
        dut.expect(re.compile(rb"DONE"), timeout=180)

        # 2. STRICT: GET is followed; body of /target is delivered.
        dut.write(b"MODE 1\n")
        dut.expect(re.compile(rb"MODE_OK 1"), timeout=180)
        dut.write(f"GET {base}/once\n".encode())
        dut.expect(re.compile(rb"CODE=200"), timeout=180)
        dut.expect(re.compile(rb"BODY:" + re.escape(FINAL_BODY)), timeout=180)
        dut.expect(re.compile(rb"DONE"), timeout=180)

        # 3. FORCE: 4-hop chain via root-relative Locations resolves to body.
        dut.write(b"MODE 2\n")
        dut.expect(re.compile(rb"MODE_OK 2"), timeout=180)
        ...
        #    Location header.
        dut.write(b"LIMIT 2\n")
        dut.expect(re.compile(rb"LIMIT_OK 2"), timeout=180)
        dut.write(f"GET {base}/loop\n".encode())
        dut.expect(re.compile(rb"CODE=302"), timeout=180)
        dut.expect(re.compile(rb"LOC=/loop"), timeout=180)
        dut.expect(re.compile(rb"DONE"), timeout=180)
    finally:
        server.shutdown()
        server.server_close()
