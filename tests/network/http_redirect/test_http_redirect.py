...
def test_http_redirect(dut):
    ...
    try:
        ...
        #    Location header.
        dut.write(b"LIMIT 2\n")
        dut.expect(re.compile(rb"LIMIT_OK 2"), timeout=10)
        dut.write(f"GET {base}/loop\n".encode())
        dut.expect(re.compile(rb"CODE=302"), timeout=10)
        dut.expect(re.compile(rb"LOC=/loop"), timeout=10)
        dut.expect(re.compile(rb"DONE"), timeout=10)
    finally:
        server.shutdown()
        server.server_close()
