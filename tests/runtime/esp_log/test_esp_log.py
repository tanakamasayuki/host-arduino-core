def test_esp_log(dut):
    ...
    dut.expect("CORE_DEBUG_LEVEL=0", timeout=30)
    dut.expect("ESP_LOG_NONE=0", timeout=30)
    dut.expect("ESP_LOG_VERBOSE=5", timeout=30)
    dut.expect("BEFORE", timeout=30)
    # No log lines should appear between BEFORE and AFTER — esp_log_level_get
    # returns NONE regardless of the level_set call.
    dut.expect("level_get=0", timeout=30)
    dut.expect("AFTER", timeout=30)
