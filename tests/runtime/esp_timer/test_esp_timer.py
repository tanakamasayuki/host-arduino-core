...
def expect_status(dut, name):
    match = dut.expect(re.compile((rf"{name}=(ok|fail)").encode()), timeout=10)
...
def test_esp_timer(dut):
    ...
    fired = dut.expect(re.compile(rb"fired=(\d+)"), timeout=10)
    expect_status(dut, "fired_lower")
    expect_status(dut, "fired_upper")
    after_stop = dut.expect(re.compile(rb"after_stop_count=(\d+)"), timeout=10)
