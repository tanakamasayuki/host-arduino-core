...
namespace
{
    ...
    const unsigned long kDefaultStartDelayMs = 250;
    ...
    HOST_ARDUINO_UNUSED bool waitForClient()
    {
        ...
        const unsigned long start_delay_ms =
            envUnsignedLong("HOST_ARDUINO_START_DELAY_MS", kDefaultStartDelayMs);
        if (start_delay_ms > 0)
        {
            std::ostringstream oss;
            oss << "delay_ms=" << start_delay_ms;
            logLine(LOG_INFO, "runtime_start_delay", oss.str());
